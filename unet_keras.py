...
montage_rgb = lambda x: np.stack([montage(x[:, :, :, i]) for i in range(x.shape[3])], -1)#从三维输入数组创建一个二维“蒙太奇”，表示同样形状的二维图像的集合。
ship_dir = '/media/wrc/8EF06A4CF06A3A9B/kaggle_data/train_orin/'
...
def multi_rle_encode(img, **kwargs):
    ...
    '''
    labels = label(img)#参数input表示需要处理的二值图像，connectivity表示判定连通的模式（1代表4连通，2代表8连通），输出labels为一个从0开始的标记数组。
    if img.ndim > 2:
        return [rle_encode(np.sum(labels==k, axis=2), **kwargs) for k in np.unique(labels[labels>0])]
    else:
        return [rle_encode(labels==k, **kwargs) for k in np.unique(labels[labels>0])]#对于一维数组或者列表，unique函数去除其中重复的元素，并按元素由大到小返回一个新的无元素重复的元组或者列表
...
def masks_as_image(in_mask_list):
    ...
    for mask in in_mask_list:
        if isinstance(mask, str):
            all_masks |= rle_decode(mask)#按位或运算符
    return all_masks
...
masks = pd.read_csv(os.path.join('/media/wrc/8EF06A4CF06A3A9B/kaggle_data/train_orin/', 'train_ship_segmentations.csv'))
not_empty = pd.notna(masks.EncodedPixels)
print(not_empty.sum(), 'masks in', masks[not_empty].ImageId.nunique(), 'images')#nuinque()是查看该序列(axis=0/1对应着列或行)的不同值的数量。用这个函数可以查看数据有多少个不同值。
print((~not_empty).sum(), 'empty images in', masks.ImageId.nunique(), 'total images')
masks.head()

#check step
# fig, (ax1, ax2, ax3, ax4) = plt.subplots(1, 4, figsize = (16, 5))
# rle_0 = masks.query('ImageId=="00021ddc3.jpg"')['EncodedPixels']#query查询符合条件的值并返回制定的列值
# #例子：df.query('a > b') 相当于 df[df.a > df.b]  # same result as the previous expression
...
unique_img_ids = masks.groupby('ImageId').agg({'ships': 'sum'}).reset_index()#把名称相同的图片合并，目标ships加到一起
...
ax_unique=unique_img_ids['file_size_kb'].hist()
fig=ax_unique.get_figure()
fig.savefig('hist1.png')
masks.drop(['ships'], axis=1, inplace=True)
unique_img_ids.sample(7)#随机采样7行

#Examine Number of Ship Images
unique_img_ids['ships'].hist(bins=unique_img_ids['ships'].max())

#Undersample Empty Images 对空图像进行了采样，以获得一个更平衡的组，并尝试使用更多的船只进行分割
...
train_ids, valid_ids = train_test_split(balanced_train_df,
                 test_size = 0.2,
                 stratify = balanced_train_df['ships'])#若不为None时，划分出来的测试集或训练集中，其类标签的比例同输入的数组中类标签的比例相同，可以用于处理不均衡的数据集。
train_df = pd.merge(masks, train_ids)##没有指定连接键，默认用重叠列名，没有指定连接方式
